...
def render_page():
    ...
    <style>
        ...
        [data-testid="stSidebar"] .stRadio [role="radiogroup"] > label:nth-child(2) p::before {{ content: '\\f468'; font-family: 'Font Awesome 6 Free'; font-weight: 900; width: 22px; text-align: center; color: inherit; transition: 0.3s; }}
        [data-testid="stSidebar"] .stRadio [role="radiogroup"] > label:nth-child(3) p::before {{ content: '\\f1da'; font-family: 'Font Awesome 6 Free'; font-weight: 900; width: 22px; text-align: center; color: inherit; transition: 0.3s; }}
        [data-testid="stSidebar"] .stRadio [role="radiogroup"] > label:nth-child(4) p::before {{ content: '\\f2c2'; font-family: 'Font Awesome 6 Free'; font-weight: 900; width: 22px; text-align: center; color: inherit; transition: 0.3s; }}
    ...
    # KIỂM TRA BẢO MẬT: CHỈ TIN TƯỞNG SESSION_STATE
    if "customer" not in st.session_state or str(st.session_state.role) != "3":
    ...
    # =========================================================
    # BỘ ĐỊNH TUYẾN THEO TAB (LAZY LOADING NGUYÊN BẢN)
    # =========================================================
    ...
    if menu_selection == "Đặt đơn hàng":
        st.markdown(f"""
            <div style="display: flex; align-items: center; margin-bottom: 10px; z-index: 2; position: relative;">
                ...
                <h1 style="margin: 0; font-size: 45px; font-weight: 700; color: white;">Tạo đơn vận chuyển (Từ kho)</h1>
            </div>
            ...
            if st.session_state.last_created_id:
                ...
                if st.session_state.temp_lat and st.session_state.temp_lon:
                    ...
                    if not st.session_state.show_payment:
                        ...
                        description = f"Thanh toan don hang {st.session_state.customer}"
                        ...
                        if st.button("ĐÃ CHUYỂN KHOẢN - TẠO ĐƠN", type="primary", use_container_width=True):
                            with st.spinner("Đang xác thực và gửi yêu cầu..."):
                                try:
                                    conn = pyodbc.connect(CONN_STR); cursor = conn.cursor()
                                    cursor.execute("""
                                        INSERT INTO LogisticsPoints (lat, lon, status, created_by, created_at, delivery_status, order_type) 
                                        OUTPUT INSERTED.point_id
                                        VALUES (?, ?, N'Chờ Admin duyệt', ?, GETDATE(), N'Chờ xếp xe', N'chuỗi')
                                    """, (st.session_state.temp_lat, st.session_state.temp_lon, st.session_state.customer))
    ...
    # 2. ĐẶT ĐƠN COD (LẺ)
    # ---------------------------------------------------------
    elif menu_selection == "Đặt đơn COD (Lẻ)":
        import customercod
        customercod.render_cod_page()
