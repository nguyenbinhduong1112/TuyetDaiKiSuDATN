...
@st.cache_data(ttl=300) 
# [CẬP NHẬT 1]: Hàm vẫn nhận username DB, nhưng khi gọi sẽ truyền customer vào
...
def render_page():
    ...
    # KIỂM TRA BẢO MẬT
    # [CẬP NHẬT 2]: Đổi 'user' thành 'customer' để nhận URL param
    ...
    # [CẬP NHẬT 3]: Kiểm tra tồn tại biến customer thay vì username
    ...
    # [CẬP NHẬT 4]: Truyền biến customer để lấy tên đầy đủ
    ...
    with col_user:
        with st.popover(f"{user_fullname}", use_container_width=True):
            ...
            if st.button("Đăng xuất", use_container_width=True, type="primary"):
                # [CẬP NHẬT 5]: Đăng xuất xóa biến customer
    ...
    if menu_selection == "Đặt đơn hàng":
            ...
            # Form nhập liệu bình thường (khi chưa tạo đơn)
                ...
                if st.session_state.temp_lat and st.session_state.temp_lon:
                    ...
                    if not st.session_state.show_payment:
                        ...
                        # [CẬP NHẬT 6]: Đổi nội dung description dùng st.session_state.customer
                        ...
                        if st.button("ĐÃ CHUYỂN KHOẢN - TẠO ĐƠN", type="primary", use_container_width=True):
                            with st.spinner("Đang xác thực và gửi yêu cầu..."):
                                try:
                                    conn = pyodbc.connect(CONN_STR); cursor = conn.cursor()
                                    # [CẬP NHẬT 7]: Truyền st.session_state.customer vào tham số created_by
                                    cursor.execute("""
        ...
        with col_map:
            ...
            # [CẬP NHẬT 8]: Gọi cache bằng biến customer
            df_active = get_user_history_orders(st.session_state.customer)
    ...
    # ==========================================
        ...
        # [CẬP NHẬT 9]: Gọi lại hàm bằng biến customer
        df_history = get_user_history_orders(st.session_state.customer)
    ...
    # ==========================================
        ...
        # [CẬP NHẬT 10]: Hiện thông tin bằng st.session_state.customer
        st.markdown(f"""
