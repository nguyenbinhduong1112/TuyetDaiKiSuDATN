        ...
        st.markdown(f"""
            <div style="background-color: rgba(255, 204, 0, 0.15); border-left: 4px solid #ffcc00; padding: 12px 20px; border-radius: 5px; margin-bottom: 20px;">
                <span style="color: #ffcc00; font-weight: bold; font-size: 15px;">
                    <i class="fa-solid fa-bell"></i> CÓ {pending_orders} ĐƠN HÀNG ĐANG CHỜ XÁC NHẬN. VUI LÒNG CHUYỂN SANG TAB QUẢN LÝ ĐƠN HÀNG ĐỂ DUYỆT!
                </span>
            </div>
        """, unsafe_allow_html=True)
...
# ---------------------------------------------------------
    ...
    if pending_orders > 0:
        st.markdown(f"""
            <div style="background-color: rgba(255, 204, 0, 0.15); border-left: 4px solid #ffcc00; padding: 12px 20px; border-radius: 5px; margin-bottom: 20px;">
                <span style="color: #ffcc00; font-weight: bold; font-size: 15px;">
                    <i class="fa-solid fa-bell"></i> CÓ {pending_orders} ĐƠN HÀNG ĐANG CHỜ XÁC NHẬN HOÀN THÀNH TỪ TÀI XẾ!
                </span>
            </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Bấm vào đây để xem chi tiết & xử lý yêu cầu", expanded=True):
            st.markdown(f"""
                <div style="background-color: #21262d; padding: 15px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #FF4B4B;">
                    ...
                    <p style="margin: 8px 0; color: #8b949e; font-size: 15px;"><i class="fa-solid fa-box-open" style="color: #FF4B4B; width: 25px;"></i> <b>Số lượng đơn:</b> <span style="color: white;">{pending_orders} đơn hàng</span></p>
                </div>
            """, unsafe_allow_html=True)
