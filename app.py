...
# NHÚNG THƯ VIỆN FONTAWESOME 6 VÀ FIX LỖI STREAMLIT RENDER CSS (Không để dòng trống)
st.markdown(f"""
    ...
    .gmaps-btn {{ position: relative; display: flex; align-items: center; justify-content: center; background-color: #FF4B4B !important; color: white !important; padding: 0.7rem 1rem; border-radius: 8px; text-decoration: none !important; font-weight: 700; font-size: 18px; border: 1px solid #FF4B4B !important; transition: all 0.3s ease-in-out; width: 100%; box-sizing: border-box; overflow: hidden; }}
    .gmaps-btn::before {{ content: ""; position: absolute; top: 0; left: -100%; width: 100%; height: 100%; background-image: url('data:image/png;base64,{gmaps_hover_b64}'); background-size: 40px; background-repeat: no-repeat; background-position: 20px center; transition: all 0.5s cubic-bezier(0.23, 1, 0.32, 1); z-index: 1; opacity: 0; }}
    .gmaps-btn:hover {{ background-color: #FF7575 !important; border-color: #FF7575 !important; }}
    .gmaps-btn:hover::before {{ left: 0; opacity: 0.6; }}
    .btn-text-content {{ position: relative; z-index: 2; display: flex; align-items: center; gap: 8px; }}
    ...
    .qr-overlay {{ display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.85); z-index: 999999; justify-content: center; align-items: center; backdrop-filter: blur(5px); }}
    #qr-toggle:checked ~ .qr-overlay {{ display: flex !important; }}
    .qr-popup {{ background: white; padding: 20px; border-radius: 15px; position: relative; text-align: center; box-shadow: 0 0 30px rgba(0,0,0,0.8); }}
    .close-btn {{ position: absolute; top: -15px; right: -15px; background: #FF4B4B; color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; justify-content: center; align-items: center; cursor: pointer; font-size: 24px; font-weight: bold; border: 3px solid white; transition: 0.3s; }}
    .open-btn {{ display: block; background: #262730; color: white; text-align: center; border-radius: 6px; cursor: pointer; font-weight: bold; border: 1px solid #444; transition: 0.3s; }}
...
def fetch_real_data():
    try:
        ...
        conn.close()
        all_locs = [kho] + df_pts.values.tolist()
        return np.array(all_locs), len(df_pts)
    except Exception: return np.array([[18.6601, 105.6942]]), 0
...
def get_driver_fullname(username):
    ...
    except Exception: return username

def update_location(status, lat, lon):
    try:
        conn = pyodbc.connect(CONN_STR); cursor = conn.cursor()
        cursor.execute("UPDATE userstable SET current_status = ?, lat = ?, lon = ? WHERE username = ?", (status, lat, lon, st.session_state.username))
...
with col_user:
    with st.popover(f"{driver_fullname}", use_container_width=True):
        st.markdown(f"**<i class='fa-solid fa-id-badge' style='color:#FF4B4B;'></i> Tài xế:** {driver_fullname}", unsafe_allow_html=True)
...
except: logo_html = '<i class="fa-solid fa-truck-fast" style="font-size: 40px; margin-right: 15px; color: white; z-index: 2; position: relative;"></i>'

st.markdown(f"""
    <div style="display: flex; align-items: center; margin-bottom: 10px; z-index: 2; position: relative;">
        {logo_html}
        <h1 style="margin: 0; font-size: 50px; font-weight: 700; color: white;">Hệ thống vận hành tài xế UMBRELLA</h1>
    </div>
...
with col_ctrl:
    ...
    if st.button("Đồng bộ đơn hàng mới", use_container_width=True):
        ...
        st.success(f"Đã cập nhật {n_orders} đơn hàng!")
        st.rerun()

    # LOGIC NÚT XÁC NHẬN HOÀN THÀNH
    try:
        conn = pyodbc.connect(CONN_STR)
        pending_count = pd.read_sql("SELECT COUNT(*) FROM LogisticsPoints WHERE delivery_status = N'Đang chờ duyệt'", conn).iloc[0,0]
        actual_orders_on_map = len(st.session_state.locations) - 1
        conn.close()
    except Exception: pending_count, actual_orders_on_map = 0, 0

    if pending_count > 0:
        st.button("Đang chờ Admin duyệt...", disabled=True, use_container_width=True)
    elif actual_orders_on_map <= 0:
...
if st.session_state.route_indices and len(st.session_state.route_indices) > 1:
    ...
    # --- ĐÃ SỬA CẤU TRÚC URL ĐỂ XUẤT PHÁT TỪ VỊ TRÍ HIỆN TẠI (Current+Location) ---
    kho_loc = st.session_state.locations[st.session_state.route_indices[0]]
    kho_str = f"{kho_loc[0]},{kho_loc[1]}"
    
    # Nối các điểm giao hàng (Bỏ điểm kho ra khỏi danh sách này vì đã ghép thủ công phía dưới)
    ...
    # Cấu trúc URL mới: Current+Location -> KHO -> Điểm 1 -> Điểm 2 -> ... -> Điểm cuối
    gmaps_url = f"https://www.google.com/maps/dir/Current+Location/{kho_str}/{pts_str}/"
    ...
    with col_qr:
        ...
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: #1A1C24; padding: 20px; border-radius: 10px; border: 1px solid #333;">
            <div style="color: #e0e0e0; font-size: 15px; font-weight: bold; margin-bottom: 12px;"><i class="fa-solid fa-qrcode" style="color:#FF4B4B;"></i> QR Google Maps App</div>
            ...
            <div class="qr-overlay">
                <div class="qr-popup">
                    <label for="qr-toggle" class="close-btn">×</label>
                    <img src="{qr_url}" style="width: 65vh; max-width: 600px; border-radius: 10px; border: 2px solid #333;">
