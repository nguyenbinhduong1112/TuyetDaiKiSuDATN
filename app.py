...
with col_user:
    with st.popover(f"👤 {driver_fullname}", use_container_width=True):
        ...
        if st.button("🚪 Đăng xuất", use_container_width=True, type="primary"):
            ...
            st.query_params.clear() # Xóa URL khi logout
            st.rerun()
...
with col_map:
    ...
    map_data = st_folium(m, width="100%", height=550, key=f"vinh_map_{st.session_state.map_refresh_key}", returned_objects=["last_clicked"])
...
# --- 4. THÔNG SỐ & MÃ QR (BẢN CHỐT: GPS -> KHO -> ĐIỂM CUỐI) ---
if st.session_state.route_indices and len(st.session_state.route_indices) > 1:
    ...
    # URL CHUẨN: // (GPS) -> Kho -> Các điểm giao (Dừng ở điểm cuối)
    gmaps_url = f"https://www.google.com/maps/dir//{kho_loc[0]},{kho_loc[1]}/{pts}/"
